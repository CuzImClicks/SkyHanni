...
data class ArrowStackLocation(val WitheredDragonInfoType: WitheredDragonInfo, val location: LorenzVec)
...
object ArrowStackWaypoints {
    ...
    fun onDragonSpawning(event: WitheredDragonEvent.ChangeEvent) {
        ...
        closestLocation = locations
            .filter { it.WitheredDragonInfoType == dragWithHighestPrio }
    ...
    fun onRenderWorld(event: LorenzRenderWorldEvent) {
        ...
        if (closestLocation != null && !shouldTracerSpawnLocation) {
            event.drawColor(closestLocation!!.location, closestLocation!!.WitheredDragonInfoType.color, true, 0.4f)
            event.draw3DLine(
                event.exactPlayerEyeLocation(),
                closestLocation!!.location.add(0.5, 0.5, 0.5),
                closestLocation!!.WitheredDragonInfoType.color.toColor(),
                2,
                false
            )
        }
    ...
    fun onTick(event: LorenzTickEvent) {
        if ((!inDungeon() && !disableChecks) || currentDragon == null) return
